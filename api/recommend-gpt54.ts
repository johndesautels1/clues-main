/**
 * /api/recommend-gpt54 — GPT-5.4 (OpenAI) City Recommender
 *
...
const GPT_INPUT_RATE = 2.50;
const GPT_OUTPUT_RATE = 10.00;
function calculateCost(i: number, o: number): number { return (i * GPT_INPUT_RATE + o * GPT_OUTPUT_RATE) / 1_000_000; }
...
function buildRecommendationPrompt(body: RecommendRequest): string {
  ...
  return `You are CLUES Intelligence's City Recommender (GPT-5.4).
...
You MUST use your knowledge and analytical capabilities to provide recommendations based on REAL, CURRENT (2025-2026) data. Do not hallucinate.
...
4. Use REAL data — verify with your training data and analytical reasoning.
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    trackCost({ sessionId: body.sessionId, model: 'gpt-5.4', endpoint: '/api/recommend-gpt54', inputTokens, outputTokens, costUsd, durationMs }).catch(() => {});

    res.status(200).json({
      recommendations,
      metadata: { model: 'gpt-5.4', inputTokens, outputTokens, costUsd: Number(costUsd.toFixed(6)), durationMs, timestamp: new Date().toISOString() },
    });
  } catch (err) {
    const durationMs = Date.now() - startTime;
    const message = err instanceof Error ? err.message : 'Unknown error';
    console.error('[/api/recommend-gpt54] Failed:', message);
    res.status(500).json({ error: 'GPT-5.4 recommendation failed', detail: message, durationMs });
  }
}
