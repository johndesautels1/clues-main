 ...
 *
 * Assembles ALL evaluation data into a unified report data structure.
 * This is the "evidence room" — every metric, every LLM score, every
 * source citation, all in one place.
 *
 * Accepts PipelineResult + UserSession (pipeline-integrated).
 *
...
import type { UserSession, GeminiExtraction } from '../types';
import type { SmartScoreOutput, MetricSource, CategorySmartScore } from '../types/smartScore';
import type { JudgeReport, JudgeOrchestrationResult } from '../types/judge';
import type { OrchestrationResult, EvaluatorModel } from '../types/evaluation';
...
/** Extract currency info from Gemini extraction */
function buildCurrencyInfo(geminiExtraction: GeminiExtraction | null): CurrencyInfo {
  ...
  return { detected: 'UNKNOWN', budgetMin: 0, budgetMax: 0 };
}

/** Build per-LLM status entries from orchestration */
function buildLLMStatus(orchestration: OrchestrationResult): LLMStatusEntry[] {
...
function buildCategoryRollups(
  ...
  return refCity.categoryScores.map((cat: CategorySmartScore) => {
...
/**
 * Assemble all pipeline data into a unified ReportData structure.
 *
...
export function assembleReportData(
  ...
  const geminiExtraction = session.paragraphical?.extraction ?? null;
  const judgeReport = judgeResult?.finalReport ?? null;

  // 1. Entry point summary
  const entryPoints = buildEntryPointSummary(geminiExtraction, evaluation, judgeReport, pipelineResult.tier);
  ...
  // 4. LLM status
  const llmStatus = buildLLMStatus(evaluation);

  // 5. Metric lines — the core evidence
  const metricLines = buildMetricLines(smartScores);
  enrichWithPerLLMScores(metricLines, evaluation);

  // 6. Category rollups
  const categoryRollups = buildCategoryRollups(smartScores, judgeReport);

  // 7. Stats
  const stats = buildStats(smartScores, evaluation, judgeReport, judgeResult, pipelineResult);
